...
class Say:
    ...
    async def upload(self, ctx, file=None, *, comment=None):
        ...
        if os.path.isfile("data/say/upload/{}".format(file)) is True:
            ...
            await self.bot.say(
                "That file doesn't seem to exist. Make sure it is the good name, try to add the extention (especially if two files have the same name) and if you just added a new file, make sure to reload the cog by typing `"
                + ctx.prefix
                + "reload say`"
            )
    ...
    async def dm(self, ctx, user: discord.Member, *, text):
        ...
        try:
            ...
            await self.bot.say(
                "I can't send DM to this user, he may had blocked DM on this server."
            )
    ...
    async def autodelete(self, ctx):
        ...
        if not ctx.message.channel.permissions_for(ctx.message.server.me).manage_messages:
            await self.bot.say(
                "Error: I need the `Manage messages` permission to enable this function. Aborting..."
            )
            return
        else:
            pass
        ...
        if self.settings[server.id]["autodelete"] == "0":
            ...
            await self.bot.say(
                "Auto-deletion is now enabled. Note that this will only work on commands of these cog. For further options,look at `"
                + ctx.prefix
                + "modset deleterepeat`"
            )
