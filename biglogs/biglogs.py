...
class biglogs:
    def __init__(self, bot):
        ...
        self.settings_file = 'data/biglogs/settings.json'
        self.settings = dataIO.load_json(self.settings_file)
        self.ignore_file = 'data/biglogs/ignore.json'
    ...
    async def _setup_questions(self, context):
        ...
        instructions = 'Thank you for using Big Logs! However, this cog requires some setting up and a dozen or so questions will be asked.\n'
        ...
        embed = discord.Embed(title='**Welcome to the setup for Big Logs! This is the Star verison of logging, I hope you enjoy it!**', description=instructions, color=self.green)
    ...
    @commands.group(pass_context=True, name='border', aliases=['biglogs', 'polizei', 'police'])
    async def _biglogs(self, context):
        if context.invoked_subcommand is None:
            await send_cmd_help(context)

    @_biglogs.command(pass_context=True, name='setup')
    @checks.mod_or_permissions(administrator=True)
    async def _setup(self, context):
        '''
        Setup your server for Big Logs
        '''
    ...
    @_biglogs.command(pass_context=True, name='ignoremember')
    @checks.mod_or_permissions(administrator=True)
    ...
    @_biglogs.command(pass_context=True, name='ignorechannel')
    @checks.mod_or_permissions(administrator=True)
    ...
    @_biglogs.command(pass_context=True, name='warn', aliases=['strike'])
    ...
    @_biglogs.command(pass_context=True, name='kick', aliases=['boot'])
    @checks.mod_or_permissions(kick_members=True)
    async def _kick_member(self, context, member: discord.Member, *, reason):
        '''
        Kick an annoying member.
        '''
        author = context.message.author
        server = context.message.server
    ...
    @_biglogs.command(pass_context=True, name='ban', aliases=['hammer'])
    @checks.mod_or_permissions(ban_members=True)
    async def _ban_member(self, context, member: discord.Member, *, reason):
        '''
        Ban an annoyance [cheesy].
        '''
        author = context.message.author
        server = context.message.server
...
def check_folder():
    if not os.path.exists('data/biglogs'):
        print('Creating data/biglogs folder...')
        os.makedirs('data/biglogs')
...
def check_file():
    ...
    settings_file = 'data/biglogs/settings.json'
    ignore_file = 'data/biglogs/ignore.json'
    if not dataIO.is_valid_json(settings_file):
        ...
        if 'db_version' in check:
            if check['db_version'] < DB_VERSION:
                data = {}
                data['db_version'] = DB_VERSION
                print('biglogs: Database version too old, please rerun the setup!')
                dataIO.save_json(settings_file, data)
